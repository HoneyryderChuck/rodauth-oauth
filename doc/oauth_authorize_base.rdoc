...
Which params have to be carried over is not something a view has to know. +authorize_form_params+ returns one entry per field, each with the +name+, +value+ and +type+ it is rendered with, so the form renders whatever the enabled features need:
...
Scopes are the exception, as the resource owner picks those. The form renders a checkbox per entry in +authorize_scopes+, other than <tt>"offline_access"</tt> under the +oidc+ feature, which is carried over as a hidden <tt>scope[]</tt> entry because it is not granted separately.
