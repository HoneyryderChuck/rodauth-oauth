...
oauth_jwt_iat_leeway :: Clock-skew leeway, in seconds, tolerated when rejecting a JWT whose +iat+ (issued-at) claim is in the future, on the json-jwt verification path. Defaults to <tt>30</tt>. RFC 7519 section 4.1.6 treats +iat+ as informational; the ruby-jwt path rejects a future +iat+ with no leeway (see jwt/ruby-jwt#319).
