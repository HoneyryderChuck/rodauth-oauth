...
oauth_default_token_endpoint_auth_methods :: list of auth methods accepted from an oauth application which has none set (contains "client_secret_basic" and "client_secret_post" by default).
...
oauth_applications_confidential_column :: the db column where the oauth application can be marked as being confidential (otherwise public), <tt>true</tt> by default.
